...
if ($uploadOk == 0) {
    ...
    if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_dir ."/". $newfilename)) {
        ...
        $sql = $conn->query("INSERT INTO POSTS (time, name, filename, oldfilename, comment, ip) VALUES ('$time', '$username', '$newfilename', '$oldfilename', '$comment', '$ip')") or die(mysqli_error($conn));
